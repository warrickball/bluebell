...
def noise(p, period=1000):
    return 1e-5*((hash(p[0])%period+hash(p[1])%period)/period-1)

def chi2(p):
    return np.sum((lorentz(t, p) + noise(p) - y)**2/sigma**2)
...
pl.savefig('7-noisy_lorentzian.pdf')
